...
export const validate = (dataType, { validate, mask }) => {
  // If this is our own internal `result` data type, then return a middleware
  // that will store the validator into the context to be used by the success()
  // function.
  //
  ...
  if (dataType === 'result') {
    return async (ctx, next) => {
      ctx.set('__cf_requests_response_validator', { validate, mask });
      await next();
    }
  }

  // If not, fall through to the Hono validator function and let it build the
  // middleware instead.
